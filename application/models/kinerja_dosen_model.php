...
class Kinerja_dosen_model extends CI_Model
{
  function get_nilai_by_dosen($kd_dosen,$thnAkademik,$kd_semester){
    $this->db->select('d.kd_dosen, d.nm_dosen, pd.nama_prodi');
    $this->db->select("
    CASE
    ...
    $this->db->select("
    CASE
      WHEN (SELECT ikd FROM rekapitulasi_polling rp WHERE rp.kd_dosen=d.kd_dosen AND tahunAkademik='$thnAkademik' AND kd_semester='$kd_semester')>3.5 THEN '5'
      WHEN (SELECT ikd FROM rekapitulasi_polling rp WHERE rp.kd_dosen=d.kd_dosen AND tahunAkademik='$thnAkademik' AND kd_semester='$kd_semester') BETWEEN 3 AND 3.49 THEN '2'
      ELSE '0'
    END as skor_ikd
    ");
    $this->db->select("
    ...
    CASE
      ...
      /
      CASE WHEN (Praktek='y') THEN '28'  ELSE sks*7 END
      )*100>90 THEN '5'
      WHEN AVG(
      (SELECT pertemuan FROM pertemuan as p1 WHERE p1.KD_DOSEN=p.KD_DOSEN AND p1.KODE_MK=p.KODE_MK AND p1.KELAS=p.KELAS ORDER BY pertemuan DESC LIMIT 1)
      /
      ...
      ELSE '0'
    END
    FROM pertemuan as p JOIN kurikulummdp as k ON k.Kode_MK=p.KODE_MK WHERE p.KD_DOSEN=d.kd_dosen AND thnAkademik='2018/2019' AND kd_semester='2'
    )as skor_pertemuan
    ");
